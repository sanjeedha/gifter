...
<?php
    ...
    if($last_five == "") {
    	$last_five = 'Ladies Belt';
    }
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("Ladies Belt", $l) && $c > 1 && $c < 5) {
			$index = array_search('Ladies Belt', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',Ladies Belt';
    	}
    	elseif (!in_array("Ladies Belt", $l)&& $c < 5)
    	{
    		$last_five .= ',Ladies Belt';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "belt.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",belt.php";
	} else {
		$last_five_pages .= ",belt.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('belt', 1, 'http://www.sanjeedhasjsu.com/belt.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/kids/belt.jpeg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">WOMEN &nbsp;BELT</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=5";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>NEOGRAM 30MM</span>
	                </p>
	                <p>
	                	<span>Measurements based on full length of belt strap. Recommended to purchase one size up.</span>
	                </p>
	                <p>
	                	<span>Calf leather strap</span>
	                </p>
	                <p>
	                	<span>Calf leather lining</span>
	                </p>
	                <p>
	                	<span>Brass buckle with shiny palladium finishing</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="5" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=5";         
	$result = mysql_query($query);
	if ($result)
	{
