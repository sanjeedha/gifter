...
<?php
    ...
    if($last_five == "") {
    	$last_five = 'Women Bracelet';
    }
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("Women Bracelet", $l) && $c > 1 && $c < 5) {
			$index = array_search('Women Bracelet', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',Women Bracelet';
    	}
    	elseif (!in_array("Women Bracelet", $l)&& $c < 5)
    	{
    		$last_five .= ',Women Bracelet';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "bracelet.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",bracelet.php";
	} else {
		$last_five_pages .= ",bracelet.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('bracelet', 1, 'http://www.sanjeedhasjsu.com/bracelet.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/her/charm.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">WOMEN &nbsp;BRACELET</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=6";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_description" class="fr-widget fr-text fr-wf fr_paragraph_center_black fr_hd7_description">
	            <div class="fr-text">$100</div>
	        </div>
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>5.0MM LAB-CREATED BLUE SAPPHIRE AND WHITE TOPAZ BRACELET IN STERLING SILVER - 7.25"</span>
	                </p>
	                <p>
	                	<span>Breathtaking and beautiful, this striking bracelet is an elegant addition to her jewelry wardrobe. Fashioned in sterling silver, this design pairs eye-catching lab-created blue sapphires with shimmering white topaz in a lovely rhythmic design.</span>
	                </p>
	                <p>
	                	<span>Stone Type: Topaz</span>
	                </p>
	                <p>
	                	<span>Stone Shape: Round</span>
	                </p>
	                <p>
	                	<span>Metal Type : Sterling Silver</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="6" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=6";         
	$result = mysql_query($query);
	if ($result)
	{
