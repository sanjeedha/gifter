...
';
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("Women NeckChain
", $l) && $c > 1 && $c < 5) {
			$index = array_search('Women NeckChain
', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',Women NeckChain
';
    	}
    	elseif (!in_array("Women NeckChain
", $l)&& $c < 5)
    	{
    		$last_five .= ',Women NeckChain
';
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "chain.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",chain.php";
	} else {
		$last_five_pages .= ",chain.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('neck chain', 1, 'http://www.sanjeedhasjsu.com/chain.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/her/necklace.jpeg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">WOMEN &nbsp;NECK CHAIN</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=8";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>PAVE INITIAL PENDANT</span>
	                </p>
	                <p>
	                	<span>Because wear-everyday jewelry, whether it's a precious heirloom or something sweetly symbolic, should be personal. choose your initial--or that of someone you want to keep close--and wear it with pride.</span>
	                </p>
	                <p>
	                	<span>shiny 12-karat gold plated metal with pave crystals</span>
	                </p>
	                <p>
	                	<span>lobster claw closure</span>
	                </p>
	                <p>
	                	<span>total chain length: 17"</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="8" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=8";         
	$result = mysql_query($query);
	if ($result)
	{
