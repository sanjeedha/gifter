...
<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("Clay set", $l) && $c > 1 && $c < 5) {
			$index = array_search('Clay set', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',Clay set';
    	}
    	elseif (!in_array("Clay set", $l)&& $c < 5)
    	{
    		$last_five .= ',Clay set';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "clay.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",clay.php";
	} else {
		$last_five_pages .= ",clay.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('clayset', 1, 'http://www.sanjeedhasjsu.com/clay.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/kids/clay.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">CLAY &nbsp;SET</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=9";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>SCHOOL SMART NON-TOXIC MODELING CLAY SET</span>
	                </p>
	                <p>
	                	<span>Intense colors and a soft, easy-to-knead consistency make School Smart modeling clay an excellent choice for all grade levels. Standard assortment includes 1.25 pounds each of red, yellow, blue and green.</span>
	                </p>
	                <p>
	                	<span>Sold as a set of 4 in assorted colors</span>
	                </p>
	                <p>
	                	<span>Individually wrapped 1-1/4 lb sticks</span>
	                </p>
	                <p>
	                	<span>Ideal for art, classrooms or home</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="9" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=9";         
	$result = mysql_query($query);
	if ($result)
	{
