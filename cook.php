...
<?php
    ...
    if($last_five == "") {
    	$last_five = 'cooking set';
    }
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("cooking set", $l) && $c > 1 && $c < 5) {
			$index = array_search('cooking set', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',cooking set';
    	}
    	elseif (!in_array("cooking set", $l)&& $c < 5)
    	{
    		$last_five .= ',cooking set';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "cook.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",cook.php";
	} else {
		$last_five_pages .= ",cook.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('cooking set', 1, 'http://www.sanjeedhasjsu.com/cook.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/kids/cook.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">COOKING &nbsp;SET</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=10";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_description" class="fr-widget fr-text fr-wf fr_paragraph_center_black fr_hd7_description">
	            <div class="fr-text">$40</div>
	        </div>
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>CURIOUS CHEF 27 PC FOUNDATION SET</span>
	                </p>
	                <p>
	                	<span>The Curious Chef® 27-Piece Foundation Set has everything you need for your adventures cooking with kids! It kit is a great gift idea for kids and is the perfect starter set for your aspiring chef! The set includes a wide variety of cooking tools for kids including the 4-piece child textile set with the child chef apron featuring adjustable straps and handy storage pockets, chef’s hat and two oven mitts</span>
	                </p>
	                <p>
	                	<span>Number of Pieces: 27</span>
	                </p>
	                <p>
	                	<span>Includes: Kitchen Timer, Whisk, Measuring Spoons, Rolling Pin, Measuring Cups, Turner, Bowl, Spatula, Cookie Cutters</span>
	                </p>
	                <p>
	                	<span>Care and Cleaning: Dishwasher-safe</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="10" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=10";         
	$result = mysql_query($query);
	if ($result)
	{
