<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("cufflink", $l) && $c > 1&& $c < 5) {
			$index = array_search('cufflink', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',cufflink';
    	}
    	elseif (!in_array("cufflink", $l)&& $c < 5)
    	{
    		$last_five .= ',cufflink';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "cufflink.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",cufflink.php";
	} else {
		$last_five_pages .= ",cufflink.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('cufflink', 1, 'http://www.sanjeedhasjsu.com/cufflink.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/cufflink.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">SILVER&nbsp;CUFFLINK</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=11";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>David Donahue Textured cufflinks</span>
	                </p>
	                <p>
	                	<span>Smooth and textured bars define square cuff links with rounded edges.</span>
	                </p>
	                <p>
	                	<span>• Sterling silver or gold-plated</span>
	                </p>
	                <p>
	                	<span>•	By David Donahue; made in the USA</span>
	                </p>
	                <p>
	                	<span>•	Men's Furnishings.</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="11" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=11";         
	$result = mysql_query($query);
	if ($result)
	{
