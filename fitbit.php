<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("fitbit", $l) && $c > 1 && $c < 5) {
			$index = array_search('fitbit', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',fitbit';
    	}
    	elseif (!in_array("fitbit", $l)&& $c < 5)
    	{
    		$last_five .= ',fitbit';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "fitbit.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",fitbit.php";
	} else {
		$last_five_pages .= ",fitbit.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('fitbit', 1, 'http://www.sanjeedhasjsu.com/fitbit.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/him/fitbit.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">FITBIT&nbsp;FLEX</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=14";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_description" class="fr-widget fr-text fr-wf fr_paragraph_center_black fr_hd7_description">
	            <div class="fr-text">$120</div>
	        </div>
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>FLEX WIRELESS ACTIVITY AND SLEEP TRACKER WRISTBAND - BLACK</span>
	                </p>
	                <p>
	                	<span>Slim, stylish and always by your side. Track your activity throughout the day and monitor the quality of your sleep at night.</span>
	                </p>
	                <p>
	                	<span>Small and large wristbands</span>
	                </p>
	                <p>
	                	<span>Charging cable</span>
	                </p>
	                <p>
	                	<span>Wireless sync dongle</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="14" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=14";         
	$result = mysql_query($query);
	if ($result)
	{
