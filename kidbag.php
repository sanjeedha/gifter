<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("Kids Bag", $l) && $c > 1&& $c < 5) {
			$index = array_search('Kids Bag', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',Kids Bag';
    	}
    	elseif (!in_array("Kids Bag", $l)&& $c < 5)
    	{
    		$last_five .= ',Kids Bag';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "kidbag.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",kidbag.php";
	} else {
		$last_five_pages .= ",kidbag.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('kidbag', 1, 'http://www.sanjeedhasjsu.com/kidbag.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/Kids-school-bag.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">KID'S &nbsp;BAG</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=15";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>HELLO KITTY LUNCHIE KIDS AND TODDLER LUNCH BAG </span>
	                </p>
	                <p>
	                	<span>Your little one will love sharing lunch with the Skip Hop Zoo Lunchie Kids and Toddler Insulated Lunch Bag Ladybug. Sized just right for little kids, this soft bag has a roomy, main compartment that holds sandwiches, snacks and drinks; an inside mesh pocket for lunch money, ice pack or utensils; and an insulated, wipe-clean interior that keeps everything cold.</span>
	                </p>
	                <p>
	                	<span>Material: Polyester</span>
	                </p>
	                <p>
	                	<span>Closure Style: Zipper</span>
	                </p>
	                <p>
	                	<span>Care and Cleaning: Wipe Clean with a Damp Cloth</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="15" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=15";         
	$result = mysql_query($query);
	if ($result)
	{
