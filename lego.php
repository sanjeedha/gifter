<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("LEGO set", $l) && $c > 1&& $c < 5) {
			$index = array_search('LEGO set', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',LEGO set';
    	}
    	elseif (!in_array("LEGO set", $l)&& $c < 5)
    	{
    		$last_five .= ',LEGO set';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "lego.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",lego.php";
	} else {
		$last_five_pages .= ",lego.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('lego blocks', 1, 'http://www.sanjeedhasjsu.com/lego.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/legokid.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">LEGO &nbsp;SET</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=17";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>LEGO CLASSIC MEDIUM CREATIVE BRICK BOX</span>
	                </p>
	                <p>
	                	<span>Designed with builders of all ages in mind, this LEGO Brick Box is a collection of 35 different colors that will encourage open-ended play, and inspire any imagination.</span>
	                </p>
	                <p>
	                	<span>Designed with builders of all ages in mind, this collection of LEGO bricks in 35 different colors will encourage open-ended building play, and inspire any imagination</span>
	                </p>
	                <p>
	                	<span>Great supplement set to any existing LEGO collection, this set comes in a convenient plastic storage box and includes ideas to get the building started</span>
	                </p>
	                <p>
	                	<span>Windows, eyes and lots and lots of wheels add to the fun and offer endless possibilities for creative construction and vehicle play with this Classic Medium LEGO storage box</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="17" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=17";         
	$result = mysql_query($query);
	if ($result)
	{
