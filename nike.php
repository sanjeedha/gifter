<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("nike shoe", $l) && $c > 1 && $c < 5) {
			$index = array_search('nike shoe', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',nike shoe';
    	}
    	elseif (!in_array("nike shoe", $l) && $c < 5)
    	{
    		$last_five .= ',nike shoe';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "nike.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",nike.php";
	} else {
		$last_five_pages .= ",nike.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('Nike shoes', 1, 'http://www.sanjeedhasjsu.com/nike.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/him/shoe.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">NIKE&nbsp;MEN SHOES</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=19";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>NIKE ROSCHE ONE FLEECE</span>
	                </p>
	                <p>
	                	<span>Enjoy the feel of the road beneath you with the flexible, responsive ride of the Nike running shoes. </span>
	                </p>
	                <p>
	                	<span>Cushioned footbed</span>
	                </p>
	                <p>
	                	<span>High-abrasion EVA</span>
	                </p>
	                <p>
	                	<span>Stack heights: 22mm heel/18mm forefoot</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="19" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=19";         
	$result = mysql_query($query);
	if ($result)
	{
