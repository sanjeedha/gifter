<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("3D pen", $l) && $c > 1 && $c < 5) {
			$index = array_search('3D pen', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',3D pen';
    	}
    	elseif (!in_array("3D pen", $l)&& $c < 5)
    	{
    		$last_five .= ',3D pen';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "pen.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",pen.php";
	} else {
		$last_five_pages .= ",pen.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('3D pen', 1, 'http://www.sanjeedhasjsu.com/pen.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/him/print.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">3D &nbsp;PRINTING PEN</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=20";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_description" class="fr-widget fr-text fr-wf fr_paragraph_center_black fr_hd7_description">
	            <div class="fr-text">$110</div>
	        </div>
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>3D PRINTER PEN WITH ABS MATERIAL</span>
	                </p>
	                <p>
	                	<span>This is the pen that creates three-dimensional plastic drawings. Seventy-five percent slimmer than previous generations, the pen extrudes warm thermoplastic that hardens within seconds, allowing you to create three-dimensional freehand drawings.</span>
	                </p>
	                <p>
	                	<span>Output material type: Fused deposition modeling</span>
	                </p>
	                <p>
	                	<span>Molding method: 3D modeling</span>
	                </p>
	                <p>
	                	<span>Sprinkle speed: Adjustable</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="20" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=20";         
	$result = mysql_query($query);
	if ($result)
	{
