<?php
    ...
    if($last_five == "") {
    	$last_five = 'ladies perfume';
    }
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("ladies perfume", $l) && $c > 1&& $c <5) {
			$index = array_search('ladies perfume', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',ladies perfume';
    	}
    	elseif (!in_array("ladies perfume", $l)&& $c < 5)
    	{
    		$last_five .= ',ladies perfume';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "perfume.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",perfume.php";
	} else {
		$last_five_pages .= ",perfume.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('perfume', 1, 'http://www.sanjeedhasjsu.com/perfume.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/her/perfume.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">VERSACE &nbsp;PERFUME</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=21";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>VERSACE EROS POUR FEMME</span>
	                </p>
	                <p>
	                	<span>Inspired by a mixture of Donatella Versace's favorite floral fragrances, Bright Crystal eau de toilette is a fresh, sensual blend of refreshing chilled yuzu and pomegranate mingled with soothing blossoms of peony, magnolia, and lotus flower, warmed with musk and amber</span>
	                </p>
	                <p>
	                	<span>Notes: iced accord, yuzu, pomegranate, peony, magnolia, lotus flower, acajou, vegetal amber, musk. Style: Sheer. Sensual. Luminous.</span>
	                </p>
	                <p>
	                	<span>Set includes:
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="21" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=21";         
	$result = mysql_query($query);
	if ($result)
	{
