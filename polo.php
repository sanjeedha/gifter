<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("polo tees", $l) && $c > 1&& $c < 5) {
			$index = array_search('polo tees', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',polo tees';
    	}
    	elseif (!in_array("polo tees", $l)&& $c < 5)
    	{
    		$last_five .= ',polo tees';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "polo.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",polo.php";
	} else {
		$last_five_pages .= ",polo.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('polo t-shirt', 1, 'http://www.sanjeedhasjsu.com/polo.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/him/polo.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">POLO&nbsp;MEN TEES</div>
	             <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=22";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>CLASSIC-FIT POCKET T-SHIRT</span>
	                </p>
	                <p>
	                	<span>This essential T-shirt is crafted from lightweight combed cotton jersey and tailored for a classic fit.</span>
	                </p>
	                <p>
	                	<span>Ribbed crew neckline</span>
	                </p>
	                <p>
	                	<span>Our signature embroidered pony accents the patch pocket at the left chest.</span>
	                </p>
	                <p>
	                	<span>100% cotton. Machine washable. Imported.</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="22" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=22";         
	$result = mysql_query($query);
	if ($result)
	{
