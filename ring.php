<?php
    ...
    if($last_five == "") {
    	$last_five = 'Ladies Ring';
    }
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("Ladies Ring", $l) && $c > 1&& $c < 5) {
			$index = array_search('Ladies Ring', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',Ladies Ring';
    	}
    	elseif (!in_array("Ladies Ring", $l)&& $c < 5)
    	{
    		$last_five .= ',Ladies Ring';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "ring.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",ring.php";
	} else {
		$last_five_pages .= ",ring.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('ladies ring', 1, 'http://www.sanjeedhasjsu.com/ring.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/ladiesring.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">GOLD PLATED &nbsp;LADIES RING</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=23";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>14K YELLOW GOLD 1/3 CT WEDDING RING</span>
	                </p>
	                <p>
	                	<span> Here we have joined up these two beautiful rings to create the perfect pair engagement ring and wedding ring. You can choose any other combination of rings. This is our recommended choice.</span>
	                </p>
	                <p>
	                	<span>Finish Style : polish</span>
	                </p>
	                <p>
	                	<span>Width Bottom : 2.3 mm</span>
	                </p>
	                <p>
	                	<span>Yellow Gold : 14ct</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="23" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=23";         
	$result = mysql_query($query);
	if ($result)
	{
