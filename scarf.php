<?php
    ...
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("Scarf", $l) && $c > 1&& $c < 5) {
			$index = array_search('Scarf', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',Scarf';
    	}
    	elseif (!in_array("Scarf", $l)&& $c < 5)
    	{
    		$last_five .= ',Scarf';
    	}
    }
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('women scarf', 1, 'http://www.sanjeedhasjsu.com/scarf.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/her/scarf.jpg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">WOMEN &nbsp;SCARF</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=24";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>MATHEMATICAL SILK SCARF</span>
	                </p>
	                <p>
	                	<span>A lightweight Italian-made wool and silk scarf in check</span>
	                </p>
	                <p>
	                	<span>220 x 70cm/86.6 x 27.6in</span>
	                </p>
	                <p>
	                	<span>51% wool, 49% silk</span>
	                </p>
	                <p>
	                	<span>Eyelash fringing at both ends</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="24" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=24";         
	$result = mysql_query($query);
	if ($result)
	{
