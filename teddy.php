<?php
    ...
    if($last_five == "") {
    	$last_five = 'Teddy Bear';
    }
    else
    {
    	$l = explode(",", $last_five);
    	$c = count($l);
    	if (in_array("Teddy Bear", $l) && $c > 1&& $c < 5) {
			$index = array_search('Teddy Bear', $l);
			unset($l[$index]);
    		$last_five = implode(",", $l) . ',Teddy Bear';
    	}
    	elseif (!in_array("Teddy Bear", $l)&& $c < 5)
    	{
    		$last_five .= ',Teddy Bear';
    	}
    }
	...
	if ($last_five_pages == "") {
		$last_five_pages .= "teddy.php";
	} elseif ($array_count_pages == 5) {
		$last_five_pages = implode(",", array_slice($lp, 1)) . ",teddy.php";
	} else {
		$last_five_pages .= ",teddy.php";
	}
...
mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
      ...
      $query1 = "INSERT INTO visits (`product`,`count`, `link`) VALUES ('teddy bear', 1, 'http://www.sanjeedhasjsu.com/teddy.php') ON DUPLICATE KEY UPDATE count = count+1 ";
    ...
    <div class="b-content">
        <div id="content_7" class="fr-widget fr-container fr_content_7">
            <div id="hd7_image" class="fr-widget fr-container fr_hd7_image">
            	<img src="images/gifts/kids/teddy.jpeg"/>
            </div>
	        <div id="hd7_name" class="fr-widget fr-text fr-wf fr_h4_black fr_hd7_name">
	            <div class="fr-text">TEDDY &nbsp;BEAR</div>
	            <?php

	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT ROUND(AVG(rating)) AS rating FROM reviews WHERE prod_id=26";         
	$result = mysql_query($query);
	if ($result)
	{
...
?>
	        ...
	        <div id="hd7_paragraph" class="fr-widget fr-text fr-wf fr_paragraph_center_grey fr_hd7_paragraph">
	            <div class="fr-text">
	            	<p>
	            		<span>DEALS JUMBO TEDDY 30 INCHES</span>
	                </p>
	                <p>
	                	<span>Here is a unique cute teddy holding lovable three hearts with his soft plush hands. This teddy is looking very adoring as well as innocent with three hearts with a rose bud. This teddy is a perfect gift for your dear ones on this special occasion.</span>
	                </p>
	                <p>
	                	<span>Light Peach and Red / Soft Plush </span>
	                </p>
	                <p>
	                	<span>14 inches x 10 inches</span>
	                </p>
	                <p>
	                	<span>Fabric with Acrylic Filling</span>
	                </p>
                </div>
            </div>
        </div>
    ...
    <fieldset>
	    ...
	    <input type="hidden" name="prod_id" value="26" id="product_id">
	</fieldset>
</form>
<?php
	...
	mysql_connect("mysql","sanjeedha","Sanju28021991") or die("Could not connect: " . mysql_error());
	mysql_select_db("phpmyadmin");
	$query = "SELECT * FROM reviews WHERE prod_id=26";         
	$result = mysql_query($query);
	if ($result)
	{
